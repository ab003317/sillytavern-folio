...
- **記憶助手**：自動或手選既有連線／模型，獨立測試是否能生成摘要。沒有金鑰、網址、向量參數等進階表單。
...
摘要／選頁不是內建大語言模型。自動模式優先尋找酒館 Connection Manager 已存的 Flash/Mini 等小模型連線，透過原生服務做獨立短請求，不帶角色預設。沒有適合連線時，使用目前聊天連線已列出的同供應商小模型，否則沿用主模型。可在助手頁改選連線／模型，不改主聊天全域設定。
...
0.2.0 對 SillyTavern 1.18.0 開發驗證，目前支援「聊天補全」。文字補全、工具／多媒體混合歷史保留宿主處理。普通 HTTP 可運行；需現代瀏覽器支持 WASM SIMD、Worker、IndexedDB。預設須保留 Chat History 區塊。檢索與摘要不保證所有細節都命中，釘選仍受容量限制。
