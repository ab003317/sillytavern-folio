...
「運行」儀表下方和「書頁目錄」頂端並列兩個按鈕，顯示各自處理頁數：

- **一鍵重新整理全部**：重新呼叫總結模型，重做目前聊天全部正文的摘要，包含已完成及人工修改過的摘要，再更新向量索引。
- **一鍵整理未整理的**：只補缺失、因正文／玩家背景改動而失效、或分段尚未完成的摘要；有效分段接續，已完成的有效摘要（含人工修改）不重做。只有向量未載入不算摘要未整理；沒有待補頁時按鈕停用。

兩者都包含隱藏的劇情正文與對應玩家輸入，排除帶系統類型標記的通知（旁白保留），不受目錄搜尋或篩選影響。不是刷新畫面，可能產生 API 費用；原文及隱藏狀態不變，釘選保留。打開聊天不會自動補整理舊頁。

單頁閱讀區頂端的 **重新整理此頁** 只重做指定頁，優先於較早的普通待整理頁。手動重整獨立於「自動記憶」開關：關閉自動時也會執行，不會擅自打開自動功能。若正在生成角色回覆，兩個一鍵按鈕仍可按；操作會保留所選範圍排隊，回覆結束後重新核對現存頁再開始，不必再按一次。
