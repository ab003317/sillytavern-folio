...
重新整理酒館，從 **輸入欄魔法棒 → 書頁記憶** 開啟。預設自動整理，可沿用酒館已有連線，也可在記憶助手中填寫獨立 API。更新、停用、卸載使用酒館原生擴充管理，不必重新安裝。
...
- **運行**：摘要完成率與本機向量就緒率的圓形儀表、最近發送取用摘要、一鍵重整和最近動作。圓環按正文頁數計算，沒有正文時顯示「—」。
...
額外請求可能收費：每段正文一次摘要、需要舊頁時通常一次選頁。助手測試也是真實短請求；查看已保存的取用紀錄不呼叫模型。暫停自動記憶停止自動任務，手動連線測試仍可執行。只在酒館頁面開啟時整理，不是伺服器離線排程。
...
- 已發送取用快照與待發送選頁分開：每段聊天在此瀏覽器 IndexedDB 保留最近 20 次，刷新、重整、刪最新回覆後仍顯示最近一次；來源已刪除／改文時標明狀態，樓層前移用內容指紋核對，不拿舊樓號猜。包含當時正文的快照只供查看，不會進入之後檢索；清除網站資料會移除紀錄，不跨裝置同步。舊版只可遷移仍存在的最後一次已觀測紀錄，無法補回已被舊版清掉的歷史。
...
0.6.1 對 SillyTavern 1.18.0 開發驗證，目前支援「聊天補全」。頁首的「自動記憶」滑動開關會同時顯示已開啟／已關閉；關閉只停止自動工作，手動重整仍可執行。文字補全、工具／多媒體混合歷史保留宿主處理。普通 HTTP 可運行；需現代瀏覽器支持 WASM SIMD、Worker、IndexedDB。預設須保留 Chat History 區塊。檢索與摘要不保證所有細節都命中，釘選仍受容量限制。
...
- `usage_browser.py`：圓形儀表、空狀態、發送取用紀錄、刪最新回覆／来源、刷新保留、未送出不覆蓋、較早紀錄切換、兩個 IndexedDB 連線並發保存；無付費請求。
