...
- **記憶助手**：第一層預設「全部使用酒館主 API」，總結與提取動態跟隨主模型；不同接口與進階參數預設收起。

## 三層設定

平常只需第一層：看進度、讀摘要和正文、查看最近取用；新安裝的兩個助手都使用酒館主 API，不必再填兩次模型。

- **第二層**：展開「不同 API 與模型」，才分別設定總結／提取的來源、網址、金鑰與模型。書頁目錄的「摘要與取用設定」可調摘要詳略、記錄重點、近期正文頁數和最多召回舊頁數。近期正文填 0 代表自動分配。
...
从舊版更新保留已保存的獨立 API 與模型，不強制換掉正在使用的連線；第一層可按「全部使用酒館主 API」切換，第二層可隨時恢復之前保存的獨立設定。新安裝直接使用主 API。獨立模式模型名稱留空時不發請求。向量是內建的第三種用途，不需要填寫。
