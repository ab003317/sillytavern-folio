...
- **運行**：真實自動整理進度、摘要完成率與本機向量就緒率的圓形儀表、最近仍在聊天中的角色回覆所對應的取用摘要、一鍵重整和最近動作。圓環按正文頁數計算，沒有正文時顯示「—」。
...
自動發現未整理正文時，即使主面板關閉，輸入區上方也會出現一個不搶焦點的進度浮層，顯示當前頁／分段、摘要頁數與向量頁數。「查看進度」直接打開運行頁；完成提示短暫顯示後收起，失敗則保留錯誤和「立即重試」。進度只根據引擎已完成的摘要與向量計算，不是假動畫。
...
0.7.0 對 SillyTavern 1.18.0 開發驗證，目前支援「聊天補全」。頁首的「自動記憶」滑動開關會同時顯示已開啟／已關閉；關閉只停止自動工作，手動重整仍可執行。文字補全、工具／多媒體混合歷史保留宿主處理。普通 HTTP 可運行；需現代瀏覽器支持 WASM SIMD、Worker、IndexedDB。預設須保留 Chat History 區塊。檢索與摘要不保證所有細節都命中，釘選仍受容量限制。
