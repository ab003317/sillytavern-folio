...
export const MEMORY_DEFAULTS=Object.freeze({detail:'standard',focus:'balanced',recentPages:0,recallPages:8,historyBudget:0});
...
export function memoryOptions(input={}){
    ...
    return {detail:input.detail??'standard',focus:input.focus??'balanced',recentPages:number(input.recentPages,0,0,20,true),recallPages:number(input.recallPages,8,1,8,true),historyBudget:number(input.historyBudget,0,0,200000,true)};
}
...
export function rolePrompt(role,advanced={},memory={}){
    const base=advanced.prompt||(role==='summary'?SUMMARY_SYSTEM.replace('80 至 180 字的',''):SELECT_SYSTEM);
    if(role==='selection')return base+'\n只輸出 JSON：{"ids":["候選 id"],"reasons":{"候選 id":"原因"}}；只可選提供的候選。';
    const length={brief:'50 至 100',standard:'80 至 180',detailed:'180 至 350'}[memory.detail??'standard'];
