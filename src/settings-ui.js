...
export function mountMemoryOptions(engine,container,{el,button,info}){
    ...
    for(const [key,label,help] of [['detail','摘要詳略','每段摘要的目標長度：精簡 50–100 字、標準 80–180 字、詳細 180–350 字。'],['focus','摘要重點','只記錄已發生的事件；選項和玩家願望不會被當成事實。'],['recentPages','保留近期正文頁數','0 代表依模型自動：小上下文按容量保留，長上下文約 4 頁／12k tokens，超長上下文約 6 頁／24k tokens。更早的頁一律經目錄查找。1–20 為自訂上限，至少保留最新回合。'],['recallPages','每次最多召回舊正文','0 代表依模型自動（小 3／長 6／超長 8 頁）。釘選頁另外計算，全部仍受容量限制。'],['summaryBlock','未送正文的舊頁附上摘要','近期正文和召回頁以外的已整理舊頁，按時間順序以目錄摘要放在歷史最前面，讓模型記得整段劇情。容量不足時最早的頁先改為只留標題，再不足才省略並註明頁數。'],['recallNote','書頁回顧提示','在最後一則玩家輸入前加入一段簡短提示，列出這次取回的舊正文頁、原因與一小段原文，讓模型知道哪些舊情節與本次相關。只引用原文，不發送摘要。']])section.append(field(el,info,inputs[key],label,help));
