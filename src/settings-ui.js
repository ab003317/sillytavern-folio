...
export function mountMemoryOptions(engine,container,{el,button,info}){
    ...
    const inputs={detail:select(el,[['brief','精簡'],['standard','標準（預設）'],['detailed','詳細']]),focus:select(el,[['balanced','事件與人物兼顧'],['plot','劇情、因果與線索'],['relationships','人物關係與承諾']]),recentPages:numeric(el,{min:0,max:20,placeholder:'0：依模型自動'}),recallPages:numeric(el,{min:0,max:8,placeholder:'0：依模型自動'}),recallNote:select(el,[['true','開啟（預設）'],['false','關閉']]),summaryBlock:select(el,[['true','開啟（預設）'],['false','關閉']]),termDepth:numeric(el,{min:0,max:4,placeholder:'2'}),historyBudget:numeric(el,{min:0,max:200000,placeholder:'0：自動'})};
    ...
    for(const [key,label,help] of [['detail','小總結詳略','小總結是正文的壓縮版，長度按原文比例：精簡約 6%（80–400 字）、標準約 10%（120–700 字）、詳細約 15%（180–1000 字）。簡介固定 1–2 句。'],['focus','摘要重點','只記錄已發生的事件；選項和玩家願望不會被當成事實。'],['recentPages','保留近期正文頁數','0 代表依模型自動：小上下文按容量保留，長上下文約 4 頁／12k tokens，超長上下文約 6 頁／24k tokens。更早的頁一律經目錄查找。1–20 為自訂上限，至少保留最新回合。'],['recallPages','每次最多召回舊正文','0 代表依模型自動（小 3／長 6／超長 8 頁）。釘選頁另外計算，全部仍受容量限制。'],['summaryBlock','未送正文的舊頁以小總結代替','近期正文和召回頁以外的已整理舊頁，以小總結留在各自的頁序位置，和正文一起按時間排列。容量不足時最早的頁依次降為簡介、標題，再不足才省略並註明頁數。'],['termDepth','詞條遞迴層數','用本次輸入和最近劇情比對各頁詞條後，再用命中頁的小總結去比對下一層。0 關閉遞迴，預設 2 層，最多 4 層。'],['recallNote','書頁回顧提示','在最後一則玩家輸入前加入一段簡短提示，列出這次取回的舊正文頁、原因與一小段原文，讓模型知道哪些舊情節與本次相關。只引用原文，不發送摘要。']])section.append(field(el,info,inputs[key],label,help));
