"""Actual two-button clicks with hidden story, real local vectors and synthetic LLMs."""
import json
import mimetypes
import pathlib
...
with sync_playwright() as p:
    ...
    try:
        ...
        page.evaluate("""async()=>{
          ...
          p.message.extra[KEY]=r;window.preserved=JSON.stringify(p.message);window.source=JSON.stringify(testContext.chat.map(m=>[m.mes,m.is_system]));
        ...
        assert '僅補未整理：2 頁' in run.inner_text()
        ...
        missing.click()
        assert '只整理未整理的正文' in run.inner_text()
        ...
        try:
            page.wait_for_function("[...document.querySelectorAll('.folio-rebuild-status')].some(e=>e.textContent.includes('摘要 2/2')&&e.textContent.includes('向量 2/2')&&e.textContent.includes('已完成'))", timeout=30000)
        except Exception:
            ...
            raise
        assert len(calls) == 2
        assert page.evaluate('JSON.stringify(testContext.chat[5])===preserved')
        ...
        print(json.dumps({'passed': True, 'missingCalls': 2, 'allCalls': 3, 'hiddenIncluded': True, 'manualSummaryPreservedByMissing': True, 'searchDoesNotLimitScope': True, 'queueCancelled': True, 'reloadNoRepeat': True, 'browserErrors': errors}), flush=True)
    finally:
        browser.close()
